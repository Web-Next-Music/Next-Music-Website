...
function resolveSlug(slug: string, exts: Extension[]): Extension | null {
    ...
    return (
        exts.find(
            (e) => e.name.toLowerCase().replace(/\s+/g, "-") === needle,
        ) ?? exts.find((e) => e.name.toLowerCase().includes(needle)) ?? null
    );
}
...
function renderInline(
    ...
    (() => {
        ...
        candidates.push({
            ...
            node: (
                <strong>
                    {renderInline(m[1] || m[2], base, `${key}_b`)}
                </strong>
            ),
        });
    })();
    ...
    (() => {
        ...
        candidates.push({
            ...
            inner: "",
            node: (
                <em>{renderInline(m[1] || m[2], base, `${key}_i`)}</em>
            ),
        });
    })();
        ...
        if (line.startsWith("```")) {
            ...
            nodes.push(
                <div key={i} className={styles.mdCodeBlock}>
                    {lang && (
                        <div className={styles.mdCodeLang}>{lang}</div>
                    )}
                    <pre>
                        <code>{codeLines.join("\n")}</code>
                    </pre>
        ...
        if (line.startsWith("#### ")) {
            ...
            if (imgMatches.length > 0) {
                const isBadge = (url: string) =>
                    /shields\.io|badge\.fury|badgen\.net|img\.shields/i.test(
                        url,
                    );
            ...
            {showUserJs && (
                <div className={styles.userjsBlock}>
                    <div className={styles.userjsBlockHead}>
                        <span className={styles.userjsBlockLabel}>
                            user.js
                        </span>
